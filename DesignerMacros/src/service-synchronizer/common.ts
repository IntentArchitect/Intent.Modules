 ...
 */
function tryGetTypeElement(typeRef: MacroApi.Context.ITypeReference): MacroApi.Context.IElementApi | null {
    try {
        if (!typeRef || !typeRef.isTypeFound()) {
            console.log(`[TYPE] Type reference not found or invalid`);
            return null;
        }
        const typeElement = typeRef.getType() as MacroApi.Context.IElementApi;
        if (!typeElement) {
 ...
 */
function isPrimitiveType(typeElement: MacroApi.Context.IElementApi): boolean {
    if (!typeElement) return false;
    const typeName = typeElement.getName().toLowerCase();
    return (PRIMITIVE_TYPE_NAMES as readonly string[]).includes(typeName);
}
 ...
 */
function isEnumType(typeElement: MacroApi.Context.IElementApi): boolean {
    if (!typeElement) return false;
 ...
 */
function isValueObjectType(typeElement: MacroApi.Context.IElementApi): boolean {
    if (!typeElement) return false;
 ...
 */
function isComplexType(typeElement: MacroApi.Context.IElementApi): boolean {
    if (!typeElement) return false;

...
function sanitizePreSelectedStoredProcs(preSelectedStoredProcs: string[]): string[] {
...
async function openStoredProcedureBrowseDialog(connectionString: string, preSelectedStoredProcs: string[]): Promise<string[]> {
    ...
    let inputProcs = sanitizePreSelectedStoredProcs(preSelectedStoredProcs);
...
async function importSqlStoredProcedure(element: MacroApi.Context.IElementApi): Promise<void> {
    ...
    let storedProcBrowseButton: IDynamicFormFieldConfig = {
        ...
        onClick: async (form: MacroApi.Context.IDynamicFormApi) => {
            ...
            try {
                let capturedStoredProcs = (storedProcNames.value as string).split(",").map(x => x.trim());

                const selectedProcs = await openStoredProcedureBrowseDialog(connectionStringStr, capturedStoredProcs);

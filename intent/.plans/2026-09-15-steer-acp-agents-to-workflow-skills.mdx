...
<Callout id="claude-merge" tone="decision" title="Superseded — fallback used, not the merge-aware template">

`.claude/settings.json` is a shared, developer-edited file carrying permissions, env and unrelated hooks. A safe upsert would need a new template type, not a variation on the markdown base, and no way was found to verify it wouldn't ever corrupt a developer's existing config on some edge-case merge.

Rather than ship an unverified generator against a file a bug could corrupt, the sanctioned fallback was taken instead: `GateScripts` emits `.agents/hooks/CLAUDE_SETUP.md`, documenting the one-time manual paste into `.claude/settings.json`. This is a real gap — the setup step is not tracked as done anywhere, so it does not survive a fresh install into a second repository, and nothing reminds a developer to repeat it. Revisit if this proves too easy to skip in practice.
...
<FileTree
  ...
  entries={[
    { path: "Tests/ModuleBuilderSkills/GateLogicTests.cs", change: "added", note: "Layer 1 — links the generated sources, drives `Cli.Run` against fixtures" },
    { path: "Tests/ModuleBuilderSkills/GateSmokeTests.cs", change: "added", note: "Shells out to `dotnet run gate.cs` — proves it runs, not merely that it compiles" },
    { path: "Tests/ModuleBuilderSkills/Fixtures/", change: "added", note: "Module folders with known `.imodspec`, `managed-files.xml` and git state" },
    { path: "intent/.plans/evidence/harness-block-matrix.md", change: "added", note: "Layer 2 — observed per-harness enforcement, filled in before shipping" }
  ]}
/>
...
<Checklist
  id="verify"
  items={[
    { id: "v1", label: "**Tests link the generated files** — the suite compiles `.agents/hooks/*.cs` from where the module installed them, not from a copy kept beside the tests" },
    { id: "v2", label: "**One test runs the real command** — `dotnet run gate.cs` executes and returns the expected exit code" },
    { id: "v3", label: "**Fail-closed works, and build contention does not** — a deleted script blocks; concurrent invocations of the same app do not" },
    { id: "v4", label: "**Defaults are quiet** — with `Install Agent Gate Hooks` off, nothing is emitted for any harness" },
    { id: "v5", label: "**Regeneration is idempotent** — a second Software Factory run stages nothing, and hand-edited content in `.claude/settings.json` survives it" },
    { id: "v6", label: "**The harness matrix is filled in and committed** — every row an observed result, none inferred from documentation", note: "This is where the two upstream deny-enforcement issues get settled" },
    { id: "v7", label: "**All five evals have been run at least once and their outcomes recorded**, including any that failed and what changed in response" }
  ]}
/>
...
No open design questions remain. Three risks carry into implementation:

1. **The upstream deny-enforcement issues** — verify before promising enforcement on Codex or Claude Code.
2. **Merge-aware generation** for `.claude/settings.json` — the one unsolved template problem, sequenced last with a documented fallback.
3. **Uninstall/reinstall strips Template Output assignments** — the fault fixed in step 1 will recur on every such cycle until the underlying behaviour is understood. Worth recording in `CONTEXT.md` regardless.

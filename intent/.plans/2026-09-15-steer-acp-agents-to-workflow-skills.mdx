...
6. **The gate is cwd-dependent** — see the `gate-cwd` callout. It fails closed, so it is a usability fault rather than an enforcement hole, but it locks the agent out of all editing with a misleading error, and it affects every harness template.
7. **`.claude/settings.json` merge rules are duplicated in the tests** — see the seam noted in `claude-settings-shipped`. Until the template is reachable from the test project, the two can drift apart silently.

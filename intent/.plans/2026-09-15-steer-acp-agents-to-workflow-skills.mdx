...
<Checklist
  ...
  items={[
    ...
    { id: "v6", label: "**The harness matrix is filled in and committed** — every row an observed result, none inferred from documentation", note: "Filled in, but not fully satisfied — see `intent/.plans/evidence/harness-block-matrix.md`: Claude Code is inconclusive (a known Claude Code bug, not a config error), Codex/Kiro/Cursor/OpenCode are untested. The two upstream deny-enforcement issues remain unresolved" },
    { id: "v7", label: "**All five evals have been run at least once and their outcomes recorded**, including any that failed and what changed in response", note: "2 of 5 (E2, E5 — the load-bearing pair) run for real and passed; see `intent/.plans/evidence/evals.md`. E1/E3/E4 not run as dedicated evals, though substantively covered by the Layer 1/2 work" }
  ]}
/>

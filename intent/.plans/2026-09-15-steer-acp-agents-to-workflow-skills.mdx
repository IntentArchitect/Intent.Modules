...
6. **The gate is cwd-dependent, and this is now a deliberate limitation rather than an open bug.** A hook command runs in whatever the agent's current directory happens to be, so leaving the project root stops the relative path resolving. It fails CLOSED — the agent is locked out of editing, nothing is waved through — and only the lookup of `gate.cs` is affected, never the gate's own root detection. An intermediate version anchored Claude Code's commands to its documented `${CLAUDE_PROJECT_DIR}` placeholder and that worked, but it left one harness spelled unlike the other five for a hazard none of them can solve uniformly. Reverted in favour of six identical commands; the hazard belongs in agent guidance — do not leave the project root — not in every hook config.

...
namespace Intent.Modules.ModuleBuilder.Dart.Templates.Templates.DartTemplateT4
{
    ...
    partial class DartTemplateT4Template : IntentTemplateBase<DartFileTemplateModel>
    {
        [IntentManaged(Mode.Fully)]
        public const string TemplateId = "Intent.Modules.ModuleBuilder.Dart.Templates.DartTemplateT4";

        [IntentManaged(Mode.Merge, Signature = Mode.Fully)]
        public DartTemplateT4Template(IOutputTarget outputTarget, DartFileTemplateModel model) : base(TemplateId, outputTarget, model)
        {
        }
...
{TemplateBody()}";
        ...
        {
            return @$"
{(IsForInterface() ? "interface" : "class")} <#= ClassName #> {{

...
/// </summary>
public class TypescriptCollectionFormatter : ICollectionFormatter
{
    private readonly TypescriptResolvedTypeInfo _typeInfo;
    private static readonly ConcurrentDictionary<string, TypescriptCollectionFormatter> Cache = new();

    private TypescriptCollectionFormatter(TypescriptResolvedTypeInfo typeInfo)
    {
        _typeInfo = typeInfo;
    }

    private TypescriptCollectionFormatter(string collectionFormat)
    {
        static TypescriptResolvedTypeInfo Parse(string type)
        {
            ...
            var jaggedArrays = new List<TypescriptJaggedArray>();
            while (TryGetBracketedContentFromEnd('[', ']', type, out var squareBracketsContent))
            {
                ...
                jaggedArrays.Add(new TypescriptJaggedArray(arrayDimensionCount));
            ...
            {
                return TypescriptResolvedTypeInfo.CreateForArray(
                    forResolvedType: Parse(type),
                    isNullable: isNullable,
                    jaggedArrays: jaggedArrays);
            }

            var genericTypeParameters = Array.Empty<TypescriptResolvedTypeInfo>();
    ...
    /// <summary>
    /// Returns a <see cref="TypescriptResolvedTypeInfo"/> which is the type of collection for this
    /// instance of the <see cref="TypescriptCollectionFormatter"/> of the provided
    /// <paramref name="typeInfo"/>.
    /// </summary>
    public TypescriptResolvedTypeInfo ApplyTo(TypescriptResolvedTypeInfo typeInfo)
    {
        if (_typeInfo == null)
        {
            return typeInfo;
        ...
        return TypescriptResolvedTypeInfo.Create(
            ...
            genericTypeParameters: _typeInfo.GenericTypeParameters.Count == 0
                ? Array.Empty<TypescriptResolvedTypeInfo>()
    ...
    /// <summary>
    /// Returns an instance of <see cref="TypescriptCollectionFormatter"/> constructed with the
    /// specified parameters.
    /// </summary>
    /// <remarks>
    /// A cache of <see cref="TypescriptCollectionFormatter"/> instances is first checked for an
    ...
    /// <para>
    /// If any of the values of <see cref="TypescriptResolvedTypeInfo.GenericTypeParameters"/> is null,
    /// they will be substituted by the provided <see cref="TypescriptResolvedTypeInfo"/> in the
    ...
    public static TypescriptCollectionFormatter Create(TypescriptResolvedTypeInfo typeInfo)
    {
        return Cache.GetOrAdd(typeInfo.ToString(), _ => new TypescriptCollectionFormatter(typeInfo));
    }

    /// <summary>
    /// Returns an instance of <see cref="TypescriptCollectionFormatter"/> based on the provided
    ...
    public static TypescriptCollectionFormatter Create(string collectionFormat)
    {
        return Cache.GetOrAdd(
            collectionFormat,
            _ => new TypescriptCollectionFormatter(collectionFormat));
    }

    /// <summary>
    /// Returns an instance of <see cref="TypescriptCollectionFormatter"/> for <see cref="System.Collections.Generic.IEnumerable{T}"/>
    /// </summary>
    /// <remarks>
    /// A cache of <see cref="TypescriptCollectionFormatter"/> instances is first checked for an
    /// already existing instance, if an instance is found then that is returned, otherwise a new
    /// instance is created, placed in the cache and returned.
    /// </remarks>
    public static TypescriptCollectionFormatter CreateIEnumerable()
    {
        return Create("System.Collections.Generic.IEnumerable<{0}>");
    }

    /// <summary>
    /// Returns an instance of <see cref="TypescriptCollectionFormatter"/> for <see cref="System.Collections.Generic.ICollection{T}"/>
    /// </summary>
    /// <remarks>
    /// A cache of <see cref="TypescriptCollectionFormatter"/> instances is first checked for an
    /// already existing instance, if an instance is found then that is returned, otherwise a new
    /// instance is created, placed in the cache and returned.
    /// </remarks>
    public static TypescriptCollectionFormatter CreateICollection()
    {
        return Create("System.Collections.Generic.ICollection<{0}>");
    }

    /// <summary>
    /// Returns an instance of <see cref="TypescriptCollectionFormatter"/> for <see cref="System.Collections.Generic.IList{T}"/>
    /// </summary>
    /// <remarks>
    /// A cache of <see cref="TypescriptCollectionFormatter"/> instances is first checked for an
    /// already existing instance, if an instance is found then that is returned, otherwise a new
    /// instance is created, placed in the cache and returned.
    /// </remarks>
    public static TypescriptCollectionFormatter CreateIList()
    {
        return Create("System.Collections.Generic.IList<{0}>");
    }

    /// <summary>
    /// Returns an instance of <see cref="TypescriptCollectionFormatter"/> for <see cref="System.Collections.Generic.List{T}"/>
    /// </summary>
    /// <remarks>
    /// A cache of <see cref="TypescriptCollectionFormatter"/> instances is first checked for an
    /// already existing instance, if an instance is found then that is returned, otherwise a new
    /// instance is created, placed in the cache and returned.
    /// </remarks>
    public static TypescriptCollectionFormatter CreateList()
    {
    ...
    /// <summary>
    /// Obsolete. Please use <see cref="Create(TypescriptResolvedTypeInfo)"/>
    /// </summary>
    /// <param name="typeInfo"></param>
    /// <returns></returns>
    [Obsolete(WillBeRemovedIn.Version4)]
    public static TypescriptCollectionFormatter GetOrCreate(TypescriptResolvedTypeInfo typeInfo)
    {
    ...
    /// <c>System.Collection.Generic.List&lt;T&gt;</c>.</param>
    [Obsolete(WillBeRemovedIn.Version4)]
    public static TypescriptCollectionFormatter GetOrCreate(string collectionFormat)
    {
    ...
    {
        return ApplyTo((TypescriptResolvedTypeInfo)typeInfo);
    }
}